...
for(k.neighb in 1:n.neighb){
  setEPS();
  postscript(paste("./output/","exposure","-hh-by-age-",k.neighb,".eps",sep=""));
  #pdf(paste("./output/","exposure","-hh-by-age-",k.neighb,".pdf",sep=""));
  layout(matrix(c(1,2), 2, 1, byrow = TRUE), heights=c(1,3))
  ...
  grid(nx=NA, ny=NULL,col="darkgray",lwd=1);
  legend(0,20,bty="n", cex=0.8, title="Age Group",c("0-1 year","1-2 years","2-5 years"),fill=c("antiquewhite2","antiquewhite3","antiquewhite4"))
...
for(k.age in 1:3){
  setEPS();
  postscript(paste("./output/","exposure","-hh-by-neighb-",k.age,".eps",sep=""));
  #pdf(paste("./output/","exposure","-hh-by-neighb-",k.age,".pdf",sep=""));
  par(mfrow=c(1,1));
  layout(matrix(c(1,2), 2, 1, byrow = TRUE), heights=c(1,3))
  if (k.age==1){
    par(mar=c(0.5, 4, 2, 0.5))
...
SP3D.dat1$pcolor[SP3D.dat1$Pathway==3] <- "salmon2";
...
SP3D.dat2$pcolor[SP3D.dat2$Pathway==3] <- "salmon2";
...
SP3D.dat3$pcolor[SP3D.dat3$Pathway==3] <- "salmon2";
...
setwd("~/stat/sanipath/exposure/exposure/v14/")
setEPS();
postscript(paste("./output/","exposure","-2D-scatterplot",".eps",sep=""));
...
legend(8.5,1.05,expression(bold("Age 0-1 year")),cex=1.5,bty="n");
...
legend(18, 0.75,      # location and inset
       ...
       col=c("chocolate3","antiquewhite4","salmon2","black","firebrick4","green2","blue","cyan"))
...
grid(nx=NULL, ny=NULL,col="darkgray",lwd=1);
legend(8.5,1.05,expression(bold("Age 1-2 years")),cex=1.5,bty="n");
...
plot(SP3D.dat3$env.intake[which(SP3D.dat3$Neighb==1)],1-SP3D.dat3$frac0[which(SP3D.dat3$Neighb==1)],type="p",col=SP3D.dat3$pcolor,pch=15,cex=2,xlim=c(0,20),ylim=c(0,1),
     #main="Age 2-5", 
     ylab="fraction exposed", xlab=substitute(paste("mean ",italic('E. coli'), " dose for 14 hr (CFU)",sep="")),xaxt="n")
...
grid(nx=NULL, ny=NULL,col="darkgray",lwd=1);
legend(8.5,1.05,expression(bold("Age 2-5 years")),cex=1.5,bty="n");
...
setEPS();
postscript(paste("./output/","exposure","-network-color1",".eps",sep=""),fonts=c("serif", "Palatino"));
...
for (k.neighb in 1:n.neighb){
  ...
  if (k.neighb==3 | k.neighb==4) l=matrix(c(0,-2,-3,2,-2.5,-2,-2.5,3,2.25,0.5,2,4,
                              0,2,0,-2,-1,-2,1,2,0.7,2,1.5,0),12,2);
  
  curved<-c(0,0.5,0,0,0.5,0,0,0,0,0,0,0,0);
  if (k.neighb==3 | k.neighb==4) curved<-c(0,0.5,0,0,0.5,0,0,0,0,0,0,0,0);
  
  eqarrowPlot1(edge.network, l, edge.arrow.size=weight/10, vertex.size=35, 
               edge.curved=curved,vertex.label.color="black");
  
  legend("topleft",paste(neighbourhoods[k.neighb],"age 0-1 year"),bty="n", cex=1);
}
...
for (k.neighb in 1:n.neighb){
  ...
  eqarrowPlot1(edge.network, l, edge.arrow.size=weight/10, vertex.size=35, 
               edge.curved=curved,vertex.label.color="black")
  legend("topleft",paste(neighbourhoods[k.neighb],"age 1-2 years"),bty="n", cex=1);
}
...
for (k.neighb in 1:n.neighb){
  ...
  eqarrowPlot1(edge.network, l, edge.arrow.size=weight/10,edge.width=1, vertex.size=35, 
               edge.curved=curved,vertex.label.color="black")
  legend("topleft",paste(neighbourhoods[k.neighb],"age 2-5 years"),bty="n", cex=1);
}
dev.off();
...
setEPS();
postscript(paste("./output/","exposure","-network-color2",".eps",sep=""),width=10.00, height=14.14,fonts=c("serif", "Palatino"));
par(mfrow=c(4,3),mai = c(0.05, 0, 0.05, 0));
par(mar=c(0,0,0,0));
for (k.neighb in 1:n.neighb){
  if (k.neighb==1) par(mai = c(0.2, 0, 0.3, 0));
  ...
  edge.network<-graph.data.frame(edge2, directed=T);
  #V(edge.network)$name[13]<-"NA"
  #delete_vertices(edge.network,"NA")
  ...
  eqarrowPlot1(edge.network, l, edge.arrow.size=weight/8, vertex.size=35, edge.width=1,
               edge.curved=curved,vertex.label.color="black");
  
  #legend("bottom",paste(neighbourhoods[k.neighb],"age 0-1"),bty="n", cex=1);
  titleline<-ifelse(k.neighb==1,0,-1);
  mtext(paste(neighbourhoods[k.neighb],"age 0-1 year"),side=1,outer=FALSE,line=titleline,cex=1);
  ####################################################################################;
  a2.net <- data.frame(start=as.character(Net.HH.a2[[k.neighb]]$start),end=as.character(Net.HH.a2[[k.neighb]]$end),size=as.numeric(as.character(Net.HH.a2[[k.neighb]]$size)),stringsAsFactors = FALSE)
  edge <-aggregate(a2.net$size, by=list(a2.net$start,a2.net$end), FUN=sum, na.rm=TRUE)
  edge1 <- edge[which(edge[,3]!=0),]
  ...
  mtext(paste(neighbourhoods[k.neighb],"age 1-2 years"),side=1,outer=FALSE,line=titleline,cex=1);
  ...
  mtext(paste(neighbourhoods[k.neighb],"age 2-5 years"),side=1,outer=FALSE,line=titleline,cex=1);
}
dev.off();