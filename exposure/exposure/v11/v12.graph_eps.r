...
#pdf(paste("./output1/","exposure","-hh-by-age",".pdf",sep=""));
for (k.age in 1:3){
  if (k.age==1){
    int.dirt1<-HH.1[[14]]
    int.flo1<-HH.1[[16]]
...
for(k.neighb in 1:n.neighb){
  setEPS();
  postscript(paste("./output1/","exposure","-hh-by-age-",k.neighb,".eps",sep=""));
  #pdf(paste("./output1/","exposure","-hh-by-age-",k.neighb,".pdf",sep=""));
  layout(matrix(c(1,2), 2, 1, byrow = TRUE), heights=c(1,3))
...
for(k.age in 1:3){
  setEPS();
  postscript(paste("./output1/","exposure","-hh-by-neighb-",k.age,".eps",sep=""));
  #pdf(paste("./output1/","exposure","-hh-by-neighb-",k.age,".pdf",sep=""));
  par(mfrow=c(1,1));
  layout(matrix(c(1,2), 2, 1, byrow = TRUE), heights=c(1,3))
  if (k.age==1){
    par(mar=c(0.5, 4, 2, 0.5))
...
setwd("~/stat/sanipath/exposure/exposure/v11/")
setEPS();
postscript(paste("./output1/","exposure","-2D-scatterplot",".eps",sep=""));
...
setEPS();
postscript(paste("./output1/","exposure","-network-color1",".eps",sep=""),fonts=c("serif", "Palatino"));
...
setEPS();
postscript(paste("./output1/","exposure","-network-color2",".eps",sep=""),width=10.00, height=14.14,fonts=c("serif", "Palatino"));
