...
png(file="~/stat/sanipath/exposure/exposure/v11/network7/example%03d.png", width=1600,height=900, res=200)
nsteps <- max(E(g)$time)
...
#lay.out<-matrix(c(0,4,-2.5,0.7,2,-3,-2,2,-2.5,-2,0,0,-1,2.25,1.5,0,2,-2,1,-2),10,2)
#Time loop starts
for(ti in seq(ceiling(min(E(g)$time)),total_time,dt)){
  ...
  plot(g,layout=lay.out,vertex.size=20,vertex.frame.color=V(g)$color,vertex.label.color="black",edge.width=E(g)$weight/2,edge.arrow.size=0.75,asp=9/16,margin=0)
