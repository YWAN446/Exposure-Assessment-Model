  ...
  for(k.subj in 1:n.subj[k.neighb]){
    if(!is.na(dom[ind.subj[k.neighb,k.subj,1]]) &
      ...
      if(dom[ind.subj[k.neighb,k.subj,1]]==domain &
         obs.global.attr[k.neighb,k.subj,1]==agecat) k.sel <- k.sel + 1;
    }
  }
...
sel.times <- array(NA,dim=c(n.neighb,max.subj,nmax.obs,2));
sel.durs <- array(NA,dim=c(n.neighb,max.subj,2));
sel.attr <- array(NA,dim=c(n.neighb,max.subj,nmax.obs,n.behav,max.attr));
sel.global.attr <- array(NA,dim=c(n.neighb,max.subj,n.global.attr));
sel.global.freq <- array(NA,dim=c(n.neighb,max.subj,n.global.freq));
sel.global.init <- array(NA,dim=c(n.neighb,max.subj,n.global.freq));
sel.global.cens <- array(NA,dim=c(n.neighb,max.subj,n.global.freq));
sel.chid <- array(NA,dim=c(n.neighb,max.subj,3));
for(k.neighb in 1:n.neighb){
  k.sel <- 0;
  for(k.subj in 1:n.subj[k.neighb]){
    if(!is.na(dom[ind.subj[k.neighb,k.subj,1]]) &
      ...
      sel.times[k.neighb,k.sel,,] <- times.behav[k.neighb,k.subj,,];
      sel.durs[k.neighb,k.sel,] <- times.obs[k.neighb,k.subj,];
      sel.attr[k.neighb,k.sel,,,] <- obs.attr[k.neighb,k.subj,,,];
      sel.global.attr[k.neighb,k.sel,] <- obs.global.attr[k.neighb,k.subj,];
      sel.global.freq[k.neighb,k.sel,] <- obs.global.freq[k.neighb,k.subj,];
      sel.global.init[k.neighb,k.sel,] <- obs.global.init[k.neighb,k.subj,];
      sel.global.cens[k.neighb,k.sel,] <- obs.global.cens[k.neighb,k.subj,];
      sel.chid[k.neighb,k.sel,] <- ch.id[k.neighb,k.subj,];
      }
    }
  }
}

sel.data <- list("n.neighb"=n.neighb, "nmax.subj"=max.subj,
                 "n.subj"=sel.subj, "n.obs"=sel.obs,
                 "n.comp"=n.comp, "n.behav"=n.behav,
                 "comp"=sel.comp, "behav"=sel.behav,
                 "time.obs"=sel.time,
                 "comp.mask"=comp.mask, "n.attr"=n.attr,
                 "max.attr"=max.attr,
                 "max.attr.lev"=max.attr.lev,
                 "attr"=sel.attr,"attr.mask"=attr.mask,"ind.mask"=ind.mask,
                 "n.global.attr"=n.global.attr,
                 "global.attr"=sel.global.attr,
                 "global.attr.mask"=global.attr.mask,
                 "censorlimit"=censorlimit,"n.global.freq"=n.global.freq,
                 "global.freq"=sel.global.freq,"global.cens"=sel.global.cens,
                 "mu.logr"=mu.logr,"sd.logr"=sd.logr,
                 "mu.loglambda"=mu.loglambda,"sd.loglambda"=sd.loglambda);

sel.init <- list("global.freq"=sel.global.init);
